...
function generateLogSamples() {
  ...
  for (let i = 0; i < freq_centers.length; i++) {
    ...
    let bounds;
    if (i === 0) {
      bounds = { min: log_bounds.min, max: freq_centers[i] };
    }
    else if (i === freq_centers.length - 1) {
      bounds = { min: freq_centers[i], max: log_bounds.max };
    }
    else {
      bounds = { min: freq_centers[i], max: freq_centers[i + 1] };
    }
